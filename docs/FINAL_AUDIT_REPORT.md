...
当输入含有 `1.`、`2.`、`3.` 等编号题时，系统把每个编号及其后续的文字、公式和换行视为一整道题。规划提供方被要求按原顺序返回完全相同数量的独立题目，禁止合并题目、擅自拆题或补入通用开场/行为/结束题；本地回退使用相同的边界规则。

规划提供方失败时，后端回退到上传题库或 Director 的确定性问题计划。页面明确显示使用了配置的规划提供方还是本地回退，不再伪装成同一种结果。
...
| P1 | 多道编号题被当作一个泛泛的话题，规划不可用时又补进无关通用题 | 后端按编号分组并保留续行/公式；规划提示词规定题目数、顺序和边界；旧版预览自动失效 | 编号粘贴题在提供方与本地回退两条路径都会逐题显示和执行 |
