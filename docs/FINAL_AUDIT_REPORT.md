...
Interviewer style 与 Initial pressure 现在同时进入 OpenAI 和 Gemini 的共享系统提示：它们真实改变语气、思考停顿、节奏和证据追问强度，并参与本地 Director 的 challenge/interrupt 门槛与“不知道”引导措辞；保护规则禁止这些设置增删、改写、重排或跳过计划题，因此主题链路保持不变。

约束规则：

- 低于 0.65 置信度的模型控制提议被拒绝；
- 中断根据 low/medium/high 使用 0.95/0.85/0.75 阈值；
- light/standard/deep 现在表示低/中/高理解深度：分别要求完成答案、连贯推理、原理层解释；追问次数仅保留统一安全上限；
- `move_on` 必须附带实质性当前回答；语义分类为非回答、偏题或不确定的内容会被 Director 拒绝并留在原题；
- 只有前端确认单题预算耗尽后，`explain_current` 才能获批且不会推进；前端观察到讲解转录后，`move_on_after_explanation` 才能推进到下一题；
- 模型不能直接结束会话；
- 已完成或已结束会话拒绝后续控制提议；
- 真实进度始终来自服务端题号和计划长度。
...
API Key 不再进入浏览器、`/google/live` URL 或 WebSocket 配置消息；Gemini
代理、OpenAI 临时令牌和规划请求都只从后端 `.env` 读取。

OpenAI Realtime：

- 后端创建短期 client secret；
- 与 Gemini 共用摄像头/麦克风授权、音频激活、首题提示和 Director 生命周期，只在传输层使用 WebRTC；
- 固定使用男声 Ash；
- 音频转录和隐藏工具提议经同一个 Director 审批接口推进问题状态。
...
| P2 | 设置页曾存在无法执行的浏览器规划 API | 删除浏览器规划 API 输入，规划模型统一从后端 `.env` 读取并限制为 HTTPS endpoint | 浏览器只显示就绪状态，不能保存或覆盖规划密钥 |
...
2. 服务密钥已移到后端 `.env`，浏览器只接收 provider readiness 和 OpenAI
   短期 client secret。公网部署仍需认证、租户隔离和密钥轮换。
...
2. OpenAI Realtime 已接入与 Gemini 等价的 Director 工具推进；真实麦克风、扬声器与模型行为仍需在每次协议或模型升级后真机回归。
...
设置页只显示 `.env` 的后端就绪状态，不再提供浏览器密钥输入。布局裁切
已修复，Gemini 和规划请求也不再从浏览器发送密钥。
...
3. 增加 AI 白板批注撤销和批注来源标识；关闭开关、离线待处理队列和截图坐标绑定已经完成。
4. 建立 Git 基线、自动化浏览器 E2E 和真实音频回归；归档故障恢复测试已经完成。
5. 在任何外部试用前解决认证、密钥、批量数据清理、SDK 许可和素材授权。
